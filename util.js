...
export function dur(ms) { const m = Math.max(0, Math.round(ms / MIN)); return m < 60 ? m + ' min' : Math.floor(m / 60) + 'h' + pad(m % 60); }
...
export function label(e, evs) {
  if (e.kind === 'feed') return e.src === 'bottle' ? 'Mamadeira' + (e.ml ? ' · ' + e.ml + ' ml' : '') : e.src === 'breast' ? 'Peito' : 'Mamada';
